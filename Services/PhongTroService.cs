...
namespace do_an_tot_nghiep.Services
{
    public interface IPhongTroService
    {
        ...
        Task<bool> UpdateAsync(PhongTro model);
    ...
    public class PhongTroService : IPhongTroService
    {
        ...
        {
            return _context.NhaTros.ToListAsync();
        }
        ...
        public async Task<List<PhongTro>> GetPhongByNhaAsync(int? nhaId)
        {
            ...
            return await _context.PhongTros
                .Where(p => p.NhaTroId == nhaId)
                .ToListAsync();
        }

        public Task<NhaTro?> GetNhaAsync(int nhaId)
        {
            return _context.NhaTros.FirstOrDefaultAsync(n => n.Id == nhaId);
        }
        ...
        public async Task<PhongTroCreateData?> GetCreateDataAsync(int? nhaId)
        {
            ...
            var nha = await _context.NhaTros.FindAsync(nhaId.Value);
            if (nha == null)
            {
                return null;
            }
        ...
        public async Task<bool> UpdateAsync(PhongTro model)
        {
        ...
        public async Task<DeleteMultipleResult> DeleteMultipleAsync(List<int> ids)
        {
            ...
            {
                return new DeleteMultipleResult(false, "Không thể xóa phòng đang thuê", phongDangThue);
            }

            _context.PhongTros.RemoveRange(phongs);
            await _context.SaveChangesAsync();
