...
namespace do_an_tot_nghiep.Controllers.Api
{
    ...
    public class HopDongApiController : ControllerBase
    {
        ...
        public async Task<IActionResult> Create([FromBody] CreateContractRequest req) // using a DTO
        {
            ...
            if (!req.NgayKetThuc.HasValue) return BadRequest("Ngày kết thúc là bắt buộc.");
            ...
            {
                UserId = 1, // Assume Admin for API without session
                Action = "CreateContract",
                EntityName = "Contracts",
                EntityId = hopDong.Id.ToString(),
                NewValue = $"Contract {hopDong.ContractCode} created for Room {room.Id}",
                CreatedAt = DateTime.UtcNow
            });
